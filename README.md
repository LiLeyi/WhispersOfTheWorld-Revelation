...
**Windows系统可使用env_install.bat安装环境**
...
提交一个压缩包，压缩包内有两个压缩包，分别为打包后文件压缩包、源文件压缩包

1. 打包后文件压缩包：交作业时，先使用npm run build构建，然后将dist文件夹中内容进行测试，若无问题，对dist文件夹进行压缩。
2. 源文件压缩包：将整个项目的文件压缩即可。**若内存过大**，需去除以下文件夹：

* node_modules：里面是各种依赖包的源文件，老师一般不会去配环境看开发模式下项目运行情况，所以这个可以不要
* dist：这个是打包后的项目，因为单独交了打包后的文件，所以这个可以不要
* .git：这个是git仓库有关配置，可以不打包
